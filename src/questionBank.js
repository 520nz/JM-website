...
// 当前题库（可被修改）
let QUESTION_BANK = DEFAULT_QUESTION_BANK.slice();
...
function filterByCategory(category) {
  if (!category) return QUESTION_BANK.slice();
...
function searchQuestions(text) {
  if (!text) return QUESTION_BANK.slice();
  const lowerText = text.toLowerCase();
  return QUESTION_BANK.filter(q => q.question.toLowerCase().indexOf(lowerText) !== -1);
}
