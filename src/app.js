var DEFAULT_QUESTION_BANK_DATA=[
...
var App = {
    QUESTION_BANK: DEFAULT_QUESTION_BANK_DATA.slice(),
    DEFAULT_QUESTION_BANK: DEFAULT_QUESTION_BANK_DATA.slice(),
    state: { quiz: [], idx: 0, answered: false, mode: 'quick', correctCount: 0, startTime: 0, timer: null },

    DB: {
        KEY: 'jj_quiz_v2',
        get: function() {
            var d = localStorage.getItem(App.DB.KEY);
            return d ? JSON.parse(d) : App.DB.defaults();
        },
        defaults: function() {
            return { history: [], wrong: [], stats: { total: 0, correct: 0, cats: {} } };
        },
        save: function(d) {
            localStorage.setItem(App.DB.KEY, JSON.stringify(d));
        },
        ...
        getWrong: function() {
            return App.DB.get().wrong;
        },
        findQ: function(qid) {
            for (var i = 0; i < App.QUESTION_BANK.length; i++) {
                if (App.QUESTION_BANK[i].id === qid) return App.QUESTION_BANK[i];
            }
            return null;
        }
    },
    ...
    saveQuestionBank: function() {
        localStorage.setItem('jj_question_bank', JSON.stringify(App.QUESTION_BANK));
    },
    ...
    loadQuestionBank: function() {
        ...
        if (saved) {
            try {
                App.QUESTION_BANK = JSON.parse(saved);
            } catch (e) {}
        }
    },
    ...
    updateQuestion: function(qid, q) {
        for (var j = 0; j < App.QUESTION_BANK.length; j++) {
            if (App.QUESTION_BANK[j].id === qid) {
                App.QUESTION_BANK[j].category = q.category;
                App.QUESTION_BANK[j].question = q.question;
                App.QUESTION_BANK[j].options = q.options;
                App.QUESTION_BANK[j].answer = q.answer;
                App.QUESTION_BANK[j].explanation = q.explanation;
                App.saveQuestionBank();
                return true;
            }
        }
    ...
    deleteQuestion: function(qid) {
        var beforeLength = App.QUESTION_BANK.length;
        App.QUESTION_BANK = App.QUESTION_BANK.filter(function(q) { return q.id !== qid; });
        if (App.QUESTION_BANK.length !== beforeLength) {
            App.saveQuestionBank();
            return true;
        }
        return false;
    ...
    filterQuestionsByCategory: function(category) {
        if (!category) return App.QUESTION_BANK;
        return App.QUESTION_BANK.filter(function(q) { return q.category === category; });
    },

    searchQuestions: function(keyword) {
        if (!keyword) return App.QUESTION_BANK;
        var kw = keyword.toLowerCase();
        return App.QUESTION_BANK.filter(function(q) { return q.question.toLowerCase().indexOf(kw) !== -1; });
    },

    generateQuiz: function(mode, category) {
        var count = App.getCount();
        var questions = category ? App.filterQuestionsByCategory(category) : App.QUESTION_BANK;
        return App.shuffle(questions).slice(0, count);
    },

    calculateAccuracy: function(total, correct) {
