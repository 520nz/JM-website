...
global.DEFAULT_QUESTION_BANK = JSON.parse(JSON.stringify(global.QUESTION_BANK));
...
global.DB = {
  ...
  findQ: function(qid) {
    for (var i = 0; i < global.QUESTION_BANK.length; i++) {
      if (global.QUESTION_BANK[i].id === qid) return global.QUESTION_BANK[i];
    }
    return null;
  }
};
...
global.saveQuestionBank = function() {
  localStorage.setItem('jj_question_bank', JSON.stringify(global.QUESTION_BANK));
};

global.loadQuestionBank = function() {
  var saved = localStorage.getItem('jj_question_bank');
  if (saved) {
    try {
      global.QUESTION_BANK = JSON.parse(saved);
    } catch (e) {}
  }
};
...
beforeEach(() => {
  localStorage.clear();
  global.QUESTION_BANK = JSON.parse(JSON.stringify(global.DEFAULT_QUESTION_BANK));
  global.state.quiz = [];
  global.state.idx = 0;
  global.state.correctCount = 0;
  global.state.answered = false;
  global.state.mode = 'quick';
});